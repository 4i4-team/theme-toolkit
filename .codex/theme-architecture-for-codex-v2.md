...
- variants may support `responsive`
...
- a responsive entry may:
  - reference a variant by name
  - partially override the current property
  - fully override the current property
...
2. if a variant is selected, merge variant on top of base
3. if a responsive rule applies, merge responsive rule on top of the current result
4. if the responsive rule points to a variant and also includes inline overrides, apply the referenced variant first, then inline overrides
...
- variants may contain `responsive`
