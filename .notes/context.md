...
- [ ] Port the colors subsystem to the refined contract (update `createPaletteThemeHelper` to use `buildSlice` + `interpretRecipe` instead of `buildHelpers` + `buildRecipes`). Validate against the colors example app.
